...
class navi_dicts():
    ...
    def __init__(self):
        ...
        self.scripts = {
            "list_samples_names": "", # В разработкеб
            "list_buttons_names": "",
            "list_groups_names": "SELECT id name FROM groups"
        }
...
# запись в JSON файл
def add_object_to_file(filename, dict_obj):
...
def save_new_object(dict_button, data):
    try:
        add_object_to_file(data["filename"], dict_button)
        return True
    except:
...
def get_list_groups_names():
    result = DB.script_get("SELECT id name FROM groups")
    return result


# распределение по функция получения данных
def get_samples():
    pass
...
if __name__ == "__main__":
    ...
    while True:
        ...
        match navi:
            # главное меню
            case "main":
                dialog = navi_dict.start_menu["dialog"]
            ...
            case "create_menu":
                result_dict = {}
                dialog = navi_dict.create_menu["dialog"]
                data = navi_dict.create_menu["data"]

                for step in dialog.keys():

# Вынести в отдельную функцию get_samples
                    print(f"step = {step} samples = {dialog[step]['list_samples']}")
                    if dialog[step]["list_samples"]:
                        print(dialog[step]["list_samples"]())
# ==========================
                ...
                if save_new_object(result_dict, data):
            ...
            case "create_button":
                result_dict = {}
                dialog = navi_dict.create_button["dialog"]
                data = navi_dict.create_button["data"]
